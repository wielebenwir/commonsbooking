describe('test overbooking process', () => {
    ...
    function assertFridayAndMondayAreClickable() {
        ...
        cy.get('.is-today')
            ...
            .next('.day-item')
            .next('.day-item')
            .should('not.have.class', 'is-locked');
    }

    function assertWeekendOverbooking(
        shouldbeBookable,
        unbookableClass = 'is-locked',
        tuesdayBookable = false,
    ) {
        ...
        cy.get('.is-start-date')
            .next('.day-item')
            .next('.day-item')
            .should('have.class', unbookableClass);
        //the monday
        if (shouldbeBookable) {
    ...
    it('Weekly Repetitions is not overbookable (1 day to overbook) works', function () {
        ...
        updatePostMetaAndReload(
            getLocIDForTest(testName, this.bookableLocations),
            '_cb_allow_lockdays_in_range',
            'on',
        );
