...
class BookingTest extends CustomPostTypeTest {
	...
	private $morningSlotTimeframe;
	private $afterNoonSlotTimeframe;
	...
	public function testGetStartTimeframe() {
		//simple full day booking on full day timeframe
		$this->assertEquals( $this->testTimeFrame->ID, $this->testBookingTomorrow->getStartTimeframe()->ID );

		//for multiple slots
		$this->assertEquals( $this->morningSlotTimeframe, $this->testBookingSpanningOverTwoSlots->getStartTimeframe()->ID );
	}

	public function testGetEndTimeframe() {
		//simple full day booking on full day timeframe
		$this->assertEquals( $this->testTimeFrame->ID, $this->testBookingTomorrow->getEndTimeframe()->ID );

		//for multiple slots
		$this->assertEquals( $this->afterNoonSlotTimeframe, $this->testBookingSpanningOverTwoSlots->getEndTimeframe()->ID );
	}
	...
	public function testPickupDatetime() {
		$this->assertEquals( 'July 2, 2021', $this->testBookingTomorrow->pickupDatetime() );
		...
		//Test for start in slot and return on full day (#1084)
		ClockMock::freeze( new \DateTime( '27.06.2021' ) ); //Sonntag
		$otherlocation = $this->createLocation( "location" );
		$otherItem     = $this->createItem( "item" );
		//Two timeframes with one slot each that span the weekdays
		$slotTimeframe1 = $this->createTimeframe(
			$otherlocation,
			$otherItem,
			strtotime( '28.06.2021' ), //Montag
			strtotime( '02.07.2021' ), //Freitag
		...
		$slotTimeframe2 = $this->createTimeframe(
			$otherlocation,
			$otherItem,
			strtotime( '28.06.2021' ), //Montag
			strtotime( '02.07.2021' ), //Freitag
		...
		$weekendFullDay = $this->createTimeframe(
			$otherlocation,
			$otherItem,
			strtotime( '26.06.2021' ), //Samstag
			strtotime( '27.06.2021' ), //Sonntag
		);
		...
		$booking = new Booking(
			$this->createBooking(
				...
				$otherItem,
				strtotime( 'now' ),
				strtotime( '01.07.2021' ),
				'08:00 AM',
				'02:00 PM',
			)
		);
		$this->assertEquals( 'June 27, 2021', $booking->pickupDatetime() );
		//also check return time for this test
		$this->assertEquals( 'July 1, 2021 02:00 PM - 06:00 PM', $booking->returnDatetime() );
	}

	public function testReturnDatetime() {
		$this->assertEquals( 'July 3, 2021', $this->testBookingTomorrow->returnDatetime() );
		...
		$weekendFullDay = $this->createTimeframe(
			$otherlocation,
			$otherItem,
			strtotime( '03.07.2021' ), //Samstag
			strtotime( '04.07.2021' ), //Sonntag
		);
		...
		$booking = new Booking(
			$this->createBooking(
				$otherlocation,
				$otherItem,
				strtotime( '01.07.2021 08:00 AM' ),
				strtotime( '03.07.2021' ),
				'08:00 AM',
			)
		);
		$this->assertEquals( 'July 3, 2021', $booking->returnDatetime() );

		//also check pickup time for this test
		$this->assertEquals( 'July 1, 2021 08:00 AM - 01:00 PM', $booking->pickupDatetime() );
	}
	...
	protected function setUpTestBookingOverSlotsTimeframes(): void {
		...
		$creationResult               = $this->createTwoBookableTimeframeSlotsIncludingCurrentDay( $separateLocation, $separateItem );
		$this->morningSlotTimeframe   = $creationResult[0];
		$this->afterNoonSlotTimeframe = $creationResult[1];
