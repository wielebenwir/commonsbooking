    ...
    $(function () {
        ...
        const handleRuleSelection = function () {
            ...
            for (let i = 0; i < groupFields.children().length - 1; i++) {
                ...
                let ruleSelector = currentGroup.find(
                    '#' + groupName + '_' + i + '_' + ruleSelectorID,
                );
                let ruleDescription = currentGroup
                    .find('[class*="' + ruleDescriptionID + '"]')
                    .find('.cmb2-metabox-description');
                let ruleAppliesAll = currentGroup.find('[class*="' + ruleAppliesAllID + '"]');
                let ruleAppliesCategories = currentGroup.find(
                    '[class*="' + ruleAppliesCategoriesID + '"]',
                );
        ...
        const handleAppliesToAll = function () {
            ...
            for (let i = 0; i < groupFields.children().length - 1; i++) {
                ...
                //find all of our fields
                let ruleAppliesAll = currentGroup
                    .find('[class*="' + ruleAppliesAllID + '"]')
                    .find('.cmb2-option');
                let ruleAppliesCategories = currentGroup.find(
                    '[class*="' + ruleAppliesCategoriesID + '"]',
                );
