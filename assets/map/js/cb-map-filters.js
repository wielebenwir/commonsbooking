function CB_Map_Filters($, cb_map) {
    ...
    this.init = function ($) {
        ...
        this.show_filters.cb_item_categories =
            Object.keys(cb_map.settings.filter_cb_item_categories).length > 0;

        if (
            this.show_filters.location_distance ||
            this.show_filters.item_availability ||
            this.show_filters.cb_item_categories
        ) {
            var $form = $('<form></form');
            this.$form = $form;
            ...
            var $filter_button = $(
                '<button type="button">' + cb_map.translation['FILTER'] + '</button>',
            );
            ...
            $filter_button.click(function (event) {
                ...
                if (that.show_filters.location_distance) {
                    ...
                    that.do_geo_search($address.val(), function () {
    ...
    this.do_geo_search = function (address, callback) {
        if (address.length > 0 && address != that.current_address) {
            ...
            jQuery
                .post(cb_map.settings.data_url, post_data, function (response) {
    ...
    this.init_distance_filter = function ($, $filter_options) {
        var $container = $(
            '<div class="cb-map-distance-filter"><div class="cb-map-filter-group-label">' +
                cb_map.translation['DISTANCE'] +
                '</div></div>',
        );
        ...
        var $undo_geo_search_button = $(
            '<button type="button" class="undo-geo-search no-right-radius"><span class="dashicons dashicons-no"></span></button>',
        );
        $geo_search_input_group.append($undo_geo_search_button);
        var $address = $(
            '<input type="text" class="no-left-radius no-right-radius" name="position_address" placeholder="' +
                cb_map.translation['ADDRESS'] +
                '"></input>',
        );
        $geo_search_input_group.append($address);
        var $geo_search_button = $(
            '<button class="geo-search no-left-radius"><span class="dashicons dashicons-location-alt"></span></button>',
        );
        ...
        var $distance_input = $(
            '<input name="max_distance" class="cb-map-distance" type="number" min="0" value="2.5" step="0.25"></input>',
        );
    ...
    ((this.init_availability_filter = function ($, $filter_options) {
        var $container = $(
            '<div class="cb-map-availability-filter"><div class="cb-map-filter-group-label">' +
                cb_map.translation['AVAILABILITY'] +
                '</div></div>',
        );
        var $wrapper = $('<div class="cb-map-filter-group"></div>');
        $container.append($wrapper);

        var $date_start_input = $(
            '<input type="date" name="date_start" min="' +
                cb_map.settings.filter_availability.date_min +
                '" max="' +
                cb_map.settings.filter_availability.date_max +
                '" value="' +
                cb_map.settings.filter_availability.date_min +
                '">',
        );
        var $date_end_input = $(
            '<input type="date" name="date_end" min="' +
                cb_map.settings.filter_availability.date_min +
                '" max="' +
                cb_map.settings.filter_availability.date_max +
                '"  value="' +
        ...
        (this.init_category_filter = function ($, $filter_options) {
            var $container = $(
                '<div class="cb-map-category-filter"><div class="cb-map-filter-group-label">' +
                    cb_map.translation['CATEGORIES'] +
                    '</div></div>',
            );
            var $wrapper = $('<div class="cb-map-filter-group"></div>');
            $container.append($wrapper);
            ...
            $.each(cb_map.settings.filter_cb_item_categories, function (group_index, group) {
                ...
                $.each(group.elements, function (element_index, category) {
                    ...
                    var $input = $(
                        '<input type="checkbox" name="cb_item_categories[]" id="cb_item_categories' +
                            category.cat_id +
                            '"  value="' +
                            category.cat_id +
                            '">',
                    ...
                    var $label = $(
                        '<label for="cb_item_categories' + category.cat_id + '"></label>',
                    );
    ...
    this.apply_item_category_filters = function (location_data, filters) {
        ...
        location_data.forEach(function (location) {
            ...
            items.forEach(function (item, item_index) {
                var is_valid = that.check_item_terms_against_categories(
                    item.terms,
    ...
    this.apply_item_availability_filters = function (location_data, filters) {
        ...
        location_data.forEach(function (location) {
            ...
            items.forEach(function (item) {
                item.availability = that.reduce_availability(
                    item.availability,
                    filters.date_start,
                    filters.date_end,
                );
    ...
    this.calc_availability_sequences = function (availability) {
        ...
        availability.forEach(function (day) {
            if (availability_sequences.length == 0) {
                ...
                if (
                    availability_sequences[availability_sequences.length - 1].status == day.status
                ) {
    ...
    this.get_max_free_days_in_row = function (availability) {
        ...
        availability_sequences.forEach(function (availability_sequence, seq_id) {
            ...
            if (availability_sequence.status == 'location-holiday') {
                if (
                    seq_id > 0 &&
                    availability_sequences[seq_id - 1].status == 'available' &&
                    availability_sequences[seq_id + 1] &&
                    availability_sequences[seq_id + 1].status == 'available'
                ) {
                    current_free_days_in_row += availability_sequence.count;
                }
            ...
            if (
                availability_sequence.status == 'booked' ||
                availability_sequence.status == 'partially-booked' ||
                availability_sequence.status == 'no-timeframe' ||
                seq_id == availability_sequences.length - 1
            ) {
