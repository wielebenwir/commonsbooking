function CB_Map() {
    ...
    ((cb_map.init_map = function () {
        ...
        if (this.settings.show_scale) {
            L.control
                .scale({ imperial: false, updateWhenIdle: true, position: 'topright' })
                .addTo(map);
        }
        ...
        map.setView(
            new L.LatLng(this.settings.lat_start, this.settings.lon_start),
            this.settings.zoom_start,
        );
        map.addLayer(osm);
        ...
        (cb_map.get_location_data = function (init) {
            ...
            jQuery
                .post(this.settings.data_url, data, function (response) {
        ...
        (cb_map.render_item_availability = function (availability) {
            ...
            availability.forEach(function (day) {
                ...
                markup +=
                    '<div class="cb-map-popup-item-availability-day ' +
                    day.status +
                    '">' +
                    date_string +
                    '</div>';
    ...
    cb_map.render_locations = function (data, init, center_position) {
        ...
        if (
            this.settings.max_cluster_radius == undefined ||
            0 < this.settings.max_cluster_radius < 10
        ) {
            ...
            if (this.settings.marker_cluster_icon) {
                marker_cluster_options.iconCreateFunction = function (cluster) {
                    ...
                    return new L.DivIcon({
                        html:
                            '<div class="cb-map-marker-cluster-icon" style="line-height: ' +
                            that.settings.marker_cluster_icon.size.height +
                            'px; background-image: url(' +
                            that.settings.marker_cluster_icon.url +
                            ')"><span>' +
                            child_count +
                            '</span></div>',
                        className: 'marker-cluster',
                        iconSize: new L.Point(
                            that.settings.marker_cluster_icon.size.width,
                            that.settings.marker_cluster_icon.size.height,
                        ),
                    });
                };
        ...
        jQuery.each(data, function (index, location) {
            ...
            location.items.forEach(function (item) {
                ...
                if (item.status == 'publish') {
                    ...
                    popup_items +=
                        '<b>' + item.name + '</b> - ' + cb_map.translation['COMING_SOON'];
                }
                ...
                if (item.timeframe_hints && item.timeframe_hints.length > 0) {
                    ...
                    for (var t = 0; t < item.timeframe_hints.length; t++) {
                        ...
                        var formatted_date_string = date.toLocaleDateString(
                            cb_map.settings.locale,
                            date_format_options,
                        );
                        popup_items +=
                            cb_map.translation[timeframe_hint.type.toUpperCase()] +
                            ' ' +
                ...
                if (cb_map.settings.show_item_availability && item.availability) {
                    popup_items +=
                        '<div class="cb-map-popup-item-availability">' +
                        cb_map.render_item_availability(item.availability) +
                        '</div>';
                }
            ...
            var marker_icon;
            if (
                item_statuses.includes('publish') &&
                cb_map.settings.preferred_status_marker_icon == 'publish'
            ) {
            ...
            marker.bindTooltip(item_names.toString(), {
                permanent: that.settings.marker_tooltip_permanent,
            });
            ...
            popup_content += '</div>';
            popup_content +=
                '<div  class="cb-map-location-info-address">' +
            ...
            jQuery('#location-zoom-in-' + that.settings.cb_map_id + '-' + index, popup).on(
                'click',
                function () {
                    that.map.closePopup();
                    that.map.setView(
                        new L.LatLng(location.lat, location.lon),
                        that.settings.zoom_max,
                    );
                },
            );
        });
        ...
        if (
            (!init && this.settings.marker_map_bounds_filter) ||
            (init && this.settings.marker_map_bounds_initial)
        ) {
            if (Object.keys(data).length > 0) {
                ...
                if (markers.getBounds === undefined && center_position === undefined) {
                    center_position = {
                        lat:
                            data.map((location) => location.lat).reduce((a, b) => a + b) /
                            data.length,
                        lon:
                            data.map((location) => location.lon).reduce((a, b) => a + b) /
                            data.length,
                    };
                }
                ...
                if (center_position) {
                    this.map.setView(
                        new L.LatLng(center_position.lat, center_position.lon),
                        this.settings.zoom_start,
                    );
                } else {
                    this.map.setView(
