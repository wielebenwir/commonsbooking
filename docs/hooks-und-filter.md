...
Damit User des Plugins zentrale Funktionalitäten auf sich anpassen können werden 
Wordpress [Action-Hooks](docs.wordpress.org/hooks/action) und [Filter-Hooks](docs.wordpress.org/hooks/filters) implementiert.
...
Ein Anwendungsfall für diesen Hook, stellt z.B. die Verwendung innerhalb einer Staging-Umgebung dar. Du möchtest dort Buchungs-Vorgänge einer neuen Version von Commonsbooking mit verschiedenen Zeitrahmen-Stations-Artikel-Kombinationen testen, aber gleichzeitig nicht Mails an alle möglichen Stationsbetreibende verschicken. Dann kannst das mit folgendem Filter-Hook via eingebundem Code-Snippet (gleichnamiges Plugin) oder Theme-/Plugin-Datei-Editor erreichen:
...
add_filter('commonsbooking_tag_cb_location__cb_location_email', 
  'mywebsite_cb_return_location_mail' );
```
