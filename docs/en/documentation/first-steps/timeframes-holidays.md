...
To configure holidays, you need to create a timeframe with the type
...
that means all items at the locations defined above are affected,
...
To regularly remove an item from rental, the timeframe type `Blocked (not Overbookable)` can be used.
...
for your state and year using the fields below. You can also manually add
