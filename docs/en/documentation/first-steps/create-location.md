...
can be set on the right-hand side under the "Publish" button.
...
provided to the user regarding pickup and contact details for the location:
...
* **Location admins**: Select one or more users to allow them to edit and manage this location. More about this: [Permission management](../basics/permission-management) (not translated yet)

## Overbooking settings {#overbooking}

::: info
In the following, we will use the term "Overbooking" to describe a booking period that spans days where an item can neither be picked up nor dropped off. This term is misleading and there is an [ongoing discussion on how to fix this issue.](https://github.com/wielebenwir/commonsbooking/issues/1858)
:::
...
For instance, if a user would like to book an item at a location
