...
Booking rules are just one of many ways to restrict item bookings by users.
You can also use the [timeframe settings](../first-steps/booking-timeframes-manage) to configure user roles that may book an item or restrict booking by setting a password for the [item](../first-steps/create-item).
...
in the "Restrictions" tab. Scroll all the way down. There you can add or delete rules.
...
In addition to that, all administrators and CB managers assigned to the affected item or location are always exempt.
