...
class Plugin {
	...
	public static function registerScriptsAndStyles() {
		...
		// spin.js
		wp_register_script( 'cb-spin', $base . 'spin-js/spin.min.js', array(), $versions['spin.js'] );

		// leaflet
		wp_register_script( 'cb-leaflet', $base . 'leaflet/leaflet.js', array(), $versions['leaflet'] );
		wp_register_style( 'cb-leaflet', $base . 'leaflet/leaflet.css', array(), $versions['leaflet'] );
		...
		wp_register_style(
			...
			array( 'cb-leaflet-markercluster-base' ),
			$versions['leaflet.markercluster']
		);
		...
		wp_register_script(
			...
			array( 'jquery', 'cb-jquery-overscroll', 'cb-leaflet', 'cb-leaflet-easybutton', 'cb-leaflet-markercluster', 'cb-leaflet-messagebox', 'cb-leaflet-spin', 'cb-map-filters' ),
			COMMONSBOOKING_MAP_PLUGIN_DATA['Version']
		);
		wp_register_style(
			'cb-map-shortcode',
			COMMONSBOOKING_MAP_ASSETS_URL . 'css/cb-map-shortcode.css',
			array( 'dashicons', 'cb-leaflet', 'cb-leaflet-easybutton', 'cb-leaflet-markercluster', 'cb-leaflet-messagebox' ),
			COMMONSBOOKING_MAP_PLUGIN_DATA['Version']
		);

		// vue
		wp_register_script( 'cb-vue', $base . 'vue/vue.runtime.global.prod.js', array(), $versions['vue'] );

		// commons-search
		wp_register_script(
			'cb-commons-search',
			$base . 'commons-search/commons-search.umd.js',
			array( 'cb-leaflet', 'cb-leaflet-markercluster', 'cb-vue' ),
			$versions['@commonsbooking/frontend']
		);
		wp_register_style(
			'cb-commons-search',
			$base . 'commons-search/style.css',
			array( 'cb-leaflet', 'cb-leaflet-markercluster' ),
			$versions['@commonsbooking/frontend']
		);
	}
	...
	public function savePostActions( $post_id, $post, $update ) {
		...
		$ignoredStates = array( 'auto-draft', 'draft' );
