...
class Timeframe extends CustomPostType {
	 ...
	 * @var int[]
	 */
	public static $multiDayBlockingFrames = array(
		self::REPAIR_ID,
		self::BOOKING_ID,
	);
	/**
	...
	protected function getCustomFields() {
		...
		return array(
			...
			array(
				...
				'type'          => 'title',
				'render_row_cb' => array( '\CommonsBooking\View\BookingCodes', 'renderDirectEmailRow' ),
			),
			array(
				...
				'sanitization_cb' => array( '\CommonsBooking\View\BookingCodes', 'sanitizeCronEmailCodes' ),
				'escape_cb'       => array( '\CommonsBooking\View\BookingCodes', 'escapeCronEmailCodes' ),
			),
			array(
				'name'          => esc_html__( 'Booking Codes', 'commonsbooking' ),
	...
	public function updatedPostMeta( $meta_id, $object_id, $meta_key, $meta_value ) {
		...
		if ( $meta_key == \CommonsBooking\Model\Timeframe::META_LOCATION_ID ) { // Location ID was changed, the only evidence we still have is the item ID
			...
			$orphanedBookings   = \CommonsBooking\Repository\Booking::getOrphaned( null, array( $item_id ) );
			if ( $orphanedBookings ) {
	...
	public function initHooks() {
		...
		add_action( 'cmb2_render_booking_codes_email_fields', array( '\CommonsBooking\View\BookingCodes', 'renderCronEmailFields' ), 10, 5 );
		add_action( 'cmb2_save_field_' . \CommonsBooking\View\BookingCodes::CRON_EMAIL_CODES, array( '\CommonsBooking\View\BookingCodes', 'cronEmailCodesSaved' ), 10, 3 );
