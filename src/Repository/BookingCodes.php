...
class BookingCodes {
	 ...
	 */
	protected static function lookupCode(int $timeframeId, int $itemId, int $locationId, string $date): ?BookingCode {
		global $wpdb;
		$table_name = $wpdb->prefix . self::$tablename;
	...
	public static function getCode( Timeframe $timeframe, int $itemId, int $locationId, string $date, int $advanceGenerationDays = self::ADVANCE_GENERATION_DAYS ) : ?BookingCode {
		...
		if ( $cacheItem ) {
			...
			$bookingCodeObject = static::lookupCode($timeframe->ID, $itemId, $locationId, $date);
