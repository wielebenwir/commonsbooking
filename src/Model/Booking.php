...
class Booking extends \CommonsBooking\Model\Timeframe {
	 ...
	 * @var string[]
	 */
	public static $bookingStates = array(
		'canceled',
		'confirmed',
		'unconfirmed',
	);
	...
	public function assignBookableTimeframeFields() {
		...
		if ( $timeframe ) {
			...
			foreach ( $neededMetaFields as $fieldName ) {
				...
				if ( in_array( $fieldName, array( 'start-time', 'end-time' ), true ) ) {
	...
	public function getAdjacentBookings(): ?array {
		...
		return array_filter( array( $previousAdjacent, $followingAdjacent ) );
	}
