...
trait Cache {
	 ...
	 * @throws InvalidArgumentException
	 */
	public static function clearCache( array $tags = array() ) {
		if ( ! count( $tags ) ) {
			self::getCache()->clear();
		} else {
	 ...
	 * @return void
	 */
	public static function scheduleClearCache( array $tags = array() ) {
		$event = wp_schedule_single_event( time(), self::$clearCacheHook, array( $tags ) );
	...
	private static function getShortcodeAndAttributes( $shortCode ) {
		...
		return array( $shortCode, $args );
	}

	private static $cbShortCodeFunctions = array(
