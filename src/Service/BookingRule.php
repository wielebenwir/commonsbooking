...
class BookingRule {
	 ...
	 */
	public function __construct( string $name, string $title, string $description, string $errorMessage, Closure $validationFunction, array $params = array(), array $selectParam = array(), ?Closure $errorFromArgs = null ) {
		if ( ! empty( $params ) ) {
			if ( count( $params ) > 2 ) {
	 ...
	 * @return string
	 */
	public function getErrorMessage( $args = array() ): string {
	 ...
	 * @throws Exception
	 */
	public static function checkSimultaneousBookings( Booking $booking, array $args = array(), $appliedTerms = false ): ?array {
		$userBookings = \CommonsBooking\Repository\Booking::getForUser( $booking->getUserData(), true, time(), array( 'confirmed' ) );
	 ...
	 * @throws Exception
	 */
	public static function checkChainBooking( Booking $booking, array $args = array(), $appliedTerms = false ): ?array {
	...
	private static function checkBookingRangeForDays( DateTime $startOfRange, DateTime $endOfRange, Booking $booking, $appliedTerms, int $allowedBookableDays ): ?array {
		$countedPostTypes = array( 'confirmed' );
