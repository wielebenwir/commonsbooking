...
class LocationMapAdmin
{
    ...
    {
        $cb_path = Map::get_active_plugin_directory('commons-booking.php');

        if ($cb_path) {
            $cmb_path = '../'.$cb_path.'/admin/includes/CMB2/init.php';
            require_once(COMMONSBOOKING_MAP_PATH.$cmb_path);
    ...
    public function add_metabox(array $meta_boxes)
    {
        ...
        $meta_boxes['cb_locations_map'] = array(
            'id'           => 'cb_locations_map',
            'title'        => Map::__('MAP_POSITIONING', 'commons-booking-map', 'Map Positioning'),
            ...
            'fields'       => array(
                array(
                    'id'      => 'cb-map'.'_latitude',
                    'name'    => Map::__('LATITUDE', 'commons-booking-map', 'Latitude'),
                    'type'    => 'text_small',
                    'default' => '',
                ),
                array(
                    'id'      => 'cb-map'.'_longitude',
                    'name'    => Map::__('LONGITUDE', 'commons-booking-map', 'Longitude'),
                    'type'    => 'text_small',
                    'default' => '',
                ),
                array(
                    'id'      => 'cb-map'.'_position',
                    'name'    => Map::__('POSITION', 'commons-booking-map', 'Position'),
                    'type'    => 'cb_map',
                    'default' => '',
                ),
            ),
